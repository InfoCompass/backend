...
Meta.prototype.handle = function (ctx, next) {
	...
	const lang		=	isItem
						?	(parts[1] || this.config.defaultLanguageCode || 'en')					
						:	(parts[2] || this.config.defaultLanguageCode || 'en')
	...
	try {
			...
			.then(
				function(data){
					fs.readFile(self.options.configPath+'/template.html', {encoding: 'utf-8'}, function(error, template) {
						...
						const description = data.description && (data.description[lang] || data.description[this.config.defaultLanguageCode || 'en'] || '').replace('"', "'")

						console.log(description, lang, data.description[lang], data.description && data.description[lang])
		...
		if(isPage){
			icUtils.getInterfaceTranslation(lang, 'CONTENT.HOME')
		}

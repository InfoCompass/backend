...
Meta.prototype.handle = function (ctx, next) {
	...
	console.log('META', parts, isPage, isItem, lang)
	...
	try {
			...
			.then(
				function(data){
					fs.readFile(self.options.configPath+'/template_item.html', {encoding: 'utf-8'}, function(error, template) {
						...
						const description = marked(data.description && (data.description[lang] || data.description[icUtils.config.defaultLanguageCode || 'en'] || '').replace('"', "'"))
		...
		if(isPage){
			const html = marked(icUtils.getInterfaceTranslation('CONTENT.HOME', lang))
			ctx.done(null, html+'ABC');
